...
pub fn parse_names_file(path: &Path) -> io::Result<Vec<String>> {
...
mod tests {
    use super::*;
    use crate::testutil::{temp_dir, temp_write, tempfile_with};

    #[test]
    fn parses_basic_names() {
        let p = tempfile_with("names", "张三\n李四\n王五\n");
        assert_eq!(parse_names_file(&p).unwrap(), vec!["张三", "李四", "王五"]);
    }

    #[test]
    fn ignores_blank_lines_and_comments() {
        let p = tempfile_with("names", "张三\n\n# 注释\n李四\n   \n# 第二个注释\n");
        assert_eq!(parse_names_file(&p).unwrap(), vec!["张三", "李四"]);
    }
    ...
    fn parses_all_supported_line_endings() {
        ...
        assert_eq!(parse_names_file(&p).unwrap(), vec!["甲", "乙", "丙", "丁"]);
    }
    ...
    fn missing_file_is_reported() {
        ...
        assert_eq!(
            parse_names_file(&p).unwrap_err().kind(),
            io::ErrorKind::NotFound
        );
    }

    #[test]
    fn read_errors_are_not_treated_as_empty_names() {
        let dir = temp_dir("names-read-error");
        let path = dir.join("names.txt");
        std::fs::create_dir(&path).unwrap();

        assert!(parse_names_file(&path).is_err());
    ...
    fn decodes_gbk_content() {
        ...
        assert_eq!(parse_names_file(&p).unwrap(), vec!["张三"]);
    ...
    fn ensure_creates_when_missing_and_skips_when_exists() {
        ...
        // 已存在时不覆盖
        assert_eq!(parse_names_file(&p).unwrap(), vec!["已有"]);

        let missing = crate::testutil::temp_path("names");
        std::fs::remove_file(&missing).ok();
        ensure_names_file(&missing).unwrap();
        assert!(!parse_names_file(&missing).unwrap().is_empty());
        std::fs::remove_file(&missing).ok();
    }
}
